...
public class StringCalculator {
    ...
    public int stringNumberCalculate(String input) throws Exception {
        ...
        int result = Integer.parseInt(input.substring(0,operationSymbolLocations.get(0)));
        if(operationSymbolLocations.size() == 1) {
            return calculate(result, Integer.parseInt(input.substring(operationSymbolLocations.get(0)+1)), input.charAt(operationSymbolLocations.get(0)));
        }
        for (int i = 0; i < operationSymbolLocations.size(); i++) {
            int secondTerm = getSecondTerm(input, operationSymbolLocations, i);
            result = calculate(result,secondTerm,input.charAt(operationSymbolLocations.get(i)));
        }
        return result;
    }

    private static int getSecondTerm(String input, LinkedList<Integer> operationSymbolLocations, int i) {
